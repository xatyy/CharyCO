			...
			if (String.valueOf(param[0]).toLowerCase().equals("r")) {
				...
				// read for a fixed time amount and measure time
					...
					int ios = new RandomAccess().randomReadFixedTime(PATH,
							bufferSize, runtime); // time -> runtime
	...
	class RandomAccess {
		...
		public long randomReadFixedSize(String filePath, int bufferSize,
			...
			while (counter++ < toRead) {
				...
				//generate random long [0, fileSize-bufferSize-1?] !!
				long rand = ThreadLocalRandom.current().nextLong(0, fileSize - bufferSize - 1);
				file.seek(rand);
				file.read(bytes);
			...
			file.close();
			return timer.stop() / 1000000; // ns to ms! maybe convert to double?
		}
		...
		public int randomReadFixedTime(String filePath, int bufferSize,
			...
			while (TimeUnit.toTimeUnit((System.nanoTime() - now), TimeUnit.Milli) < millis) {
